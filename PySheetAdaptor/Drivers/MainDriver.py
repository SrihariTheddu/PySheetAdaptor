...
import sqlalchemy
import time
from PySheetAdaptor.Drivers.UtilityDriver import batch_row_level_change,row_level_change, column_level_change, table_level_change, run_with_protected_environ
import csv
from PySheetAdaptor.Drivers.SystemAdminDriver import SystemAdminDriver
from PySheetAdaptor.Drivers.UtilityDriver import TransitionException, SourceEngineNotAvailableException, DestinationEngineNotAvailableException
## ====================================================================================================================
class MainDriver(SystemAdminDriver):
    """
    ...
    def install_driver(self, *args, **kwargs) -> bool:
        """
        ...
        super().setup_report_handler(*args, **kwargs)
    ...
    def load_from_file(self, source_url)-> bool:
        ...
        if path.isfile(source_url):
            ...
            raise FileNotFoundError(f"File not found on path {source_url}")
    ...
    def _load_(self) -> bool:
        if self.source_name[-3:] == "csv":
           ...
           raise SourceEngineNotAvailableException(f"No SourceEngine Available for {self.source_name}")

    def dump(self, destination_url=None, destination_name=None):
        self.destination_url = destination_url
        self.destination_name = path.basename(self.destination_url) if destination_name is None else destination_name
        try:
            return self._dump_()
        except:
    ...
    def manual_dump(self, destination_url):
        ...
        self.success.warning(f"Dumped to file successfully file url(csv): {destination_url}")
    ...
    def _map_(self):
        try:
            ...
            raise TransitionException("Error in mapping the data")
        return False
    ...
    def set_attributes(self, attributes):
        ...
        self.back_up_data.append(f"Attributes-00-"+str(reduce(lambda x, y: str(x)+":"+str(y), attributes)))
        #self._insert_(record=self.columns, index = 0)
        return True
    ...
    def get_all_indexes(self, keys: list, values: list) -> list:
        indexes = []
        if len(keys) != len(values):
            return -1
        key_indexes: list = []
    ...
    def __insert__(self, **kwargs):
        ...
        if index == None:
            ...
            self.success.warning(f"RowLevelSchema :: ExecutedFromQueue <InsertRecord::Record><{record}> at Index<{self.no_of_rows}>")
            self.record.info(f"RowLevelSchema :: ExecutedFromQueue <InsertRecord::Record><{record}> at Index<{self.no_of_rows}>")
        else:
            self.records[index] = record
            self.success.warning(f"RowLevelSchema :: ExecutedFromQueue <InsertRecord::Record><{record}> at Index<{index}>")
            self.record.info(f"RowLevelSchema :: ExecutedFromQueue <InsertRecord::Record><{record}> at Index<{index}>")
        self.no_of_rows += 1
    ...
    def _insert_(self, **kwargs) -> dict:
        ...
        if index == None:
            ...
            self.report.debug(f"RowLevelSchema :: AddedToQueue <InsertRecord::Record><{record}> at Index<{self.cursor}>")
            index = self.cursor
        else:
            action = (self.__insert__, {"record": record, "index": int(index)})
            reaction = (self.__delete_record__, {"record": record})
            self.report.debug(f"RowLevelSchema :: AddedToQueue <InsertRecord::Record><{record}> at Index<{int(index)}>")
        self.cursor += 1
        return {
            ...
            "action": action,
            "reaction": reaction,
            "url": f"/{self.source_name}/records/index={index}/append/"
        }
    ...
    def _search_record_(self, **kwargs) -> object:
        ...
        if keys == self.ARGUMENT_ERROR or values == self.ARGUMENT_ERROR:
            return self.RECORD_NOT_FOUND
    ...
    def __update__(self, **kwargs):
        ...
        self.records[index] = record
        self.success.warning(f"RowLevelSchema :: ExecutedFromQueue <UpdateRecord::Record><{record}> at Index<{self.cursor}>")
        self.record.info(f"RowLevelSchema :: ExecutedFromQueue <UpdateRecord::Record><{record}> at Index<{self.cursor}>")
    ...
    def _update_(self, **kwargs) -> dict:
        ...
        return {
            ...
            "action": action,
            "reaction": reaction,
            "url": f"/{self.source_name}/records/{str([ key+'='+str(value)+'&&' for key,value in zip(keys, values)])}/update/"
        }
    ...
    def __delete_record__(self, **kwargs):
        ...
        self.no_of_rows -= 1
        self.success.warning(f"RowLevelSchema :: ExecutedFromQueue <DeleteRecord::Record><{record}> at Index<{self.no_of_rows+1}>")
        self.record.info(f"RowLevelSchema :: ExecutedFromQueue <DeleteRecord::Record><{record}> at Index<{self.no_of_rows+1}>")
    ...
    def _delete_(self, **kwargs):
        ...
        return  {
            ...
            "action": action,
            "reaction": reaction,
            "url": f"/{self.source_name}/records/{str([ key+'='+str(value)+'&&' for key,value in zip(keys, values)])}/delete/"
        }

    def _delete_all_(self, **kwargs):
        keys = kwargs.pop("keys", self.ARGUMENT_ERROR)
        values = kwargs.pop("values", self.ARGUMENT_ERROR)
        # *** get the index of record using utility functions........
        indexes = self.get_all_indexs(keys=keys, values=values)
        for index in indexes:
            ...
            self.report.debug(f"RowLevelSchema :: AddedToQueue <DeleteRecord::Record><{record}> at Index<{index}>")
            self.cursor -= 1
            batch_row_level_change(exec_schema = {
                "Schema": "Row Level Schema",
                "Status": self.TRANSACTION_SUCCESS,
                "Operation": "Delete",
                "Index": index,
    ...
    def __add_column__(self, **kwargs):
        ...
        self.report.warning(f"ColumnLevelSchema :: ExecutedFromQueue AddColumn/column={column_name}/ ")
        self.record.info(f"ColumnLevelSchema :: ExecutedFromQueue AddColumn/column={column_name}/ ")
        self.back_up_data.append(f"Attributes-00-"+str(reduce(lambda x, y: str(x)+":"+str(y), self.columns)))

    @column_level_change
    def add_column(self, **kwargs) -> dict:
        # *** Retrieve the Data from arguments......
        column_name = kwargs.pop("column_name", self.ARGUMENT_ERROR)
        column_values = kwargs.pop("column_values", self.ARGUMENT_ERROR)
        # *** Checking if Valid Column Name....
        if column_name == self.ARGUMENT_ERROR:
    ...
    def __delete_column__(self, **kwargs):
        ...
        self.report.warning(f"ColumnLevelSchema :: ExecutedFromQueue <DeleteColumn::Column><{column_name}>")
        self.record.info(f"ColumnLevelSchema :: ExecutedFromQueue <DeleteColumn::Column><{column_name}>")
        self.back_up_data.append(f"Attributes-00-"+str(reduce(lambda x, y: str(x)+":"+str(y), self.columns)))
